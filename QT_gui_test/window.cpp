...
/* I2C FUNCTIONS */
void Window::selectDevice(int fd, int addr, char * name){
...
void Window::writeToDevice( int fd, int reg, int val, char * name ){
...
void Window::timerTimeout(){
	...
	*/
	...
	heading->setText(QString::number(hmc5883l_head));
	temp->setText(QString::number(tmp102_temperature * 0.0625));
	xaccl->setText(QString::number(adxl345_xa));
	yaccl->setText(QString::number(adxl345_ya));
	zaccl->setText(QString::number(adxl345_za));
	screenonLabel->setText(QString::number(screenon));
	eventcountLabel->setText(QString::number(eventcount));
	timer->start();
}
